...
all_data_daily <- Thun_Network_data_daily |>
  ...
  mutate(UHI_day = air_temp_LCD_day - AT,
         UHI_night = air_temp_LCD_night - AT,
...
Visualize missing data for UHI variable
```{r}
readRDS("../data/final_predictors/all_data_daily.rds") |>
  select(time, Log_NR, UHI, daynight) |>
  filter(daynight == "n") |>
  # to wide format with Log_NR as columns
  pivot_wider(names_from = Log_NR, values_from = UHI) |>
  visdat::vis_miss(warn_large_data = FALSE, sort_miss = TRUE)
ggsave("../results/Measurement Season 2024/Missing_night_UHI.png", width = 8, height = 4.5, dpi= 600)

```

```{r}
# Load the raw data
tenmin_raw <- read.csv("../data/Thun Messnetz 2024/Logger_data_T_2024-05-01_2024-10-30.csv") |>
  select(-time)
# use vismiss
visdat::vis_miss(tenmin_raw, warn_large_data = FALSE)
ggsave("../results/Measurement Season 2024/Missing_10min_raw.png", width = 8, height = 4.5, dpi= 600)
