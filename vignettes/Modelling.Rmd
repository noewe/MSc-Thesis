...
## Run Burger model for Thun for JJA (to compare to Burger et al., 2022)
...
ggplot(QM_TPI_500_Model, mapping=aes(x=UHI_est, y= UHI)) +
  geom_point(alpha = 0.2) +
    labs(x="UHI intensity predicted [K]", 
       y="UHI intensity measured [K]", 
       title= "LUR_B0_JJA", #"Interaction model TPI Thun (calibrated on Bern)", 
       subtitle = substitute(
          R^2 == r2 ~ "; RMSE =" ~ rmse ~ "K" ~ "; MBE =" ~ mbe ~ "K",
          list(r2 = R2, rmse = RMSE, mbe = MBE)
        )
  ...
  geom_abline(intercept=0, slope=1, color="red") +
  theme_bw()
ggsave("../results/models/LUR_B0_JJA/model_summary.png", width=6.5, height=4, dpi=600)
```

Data Splitting
```{r}
Masterfile <- readRDS("../data/final_predictors/all_data_daily.rds") 
visdat::vis_miss(Masterfile, warn_large_data = F)
...
QM_TPI_500_Model<-mutate(B0_test, UHI_est=6.035e-01-(8.310e-03*AD)-(2.869e-01*GA_25)+(1.006e+00*BUL_250)-(6.103e-01*AC_750)-(2.332e-02*VH_150)-(7.164e-01*FO_1000)-
                           (3.140e-03*TPI_500)-(4.913e-01*WS_S)-(9.963e-01*PP_bool)+(6.563e-03*GR_1d)+
                           (4.705e-05*(GR_1d*AD))-(1.945e-03*(GR_1d*GA_25))+(4.572e-03*(GR_1d*BUL_250))-(1.029e-02*(GR_1d*AC_750))-(1.351e-04*(GR_1d*VH_150))-(6.211e-03*(GR_1d*FO_1000))+(6.031e-05*(GR_1d*TPI_500))-
                           (1.396e-03*(WS_S*AD))-(4.788e-01*(WS_S*BUL_250))+(3.482e-01*(WS_S*AC_750))+(3.748e-01*(WS_S*FO_1000))-(8.287e-03*(WS_S*VH_150))-
                           (5.562e-03*(PP_bool*AD))+(3.294e-01*(PP_bool*GA_25))+(1.630e+00*(PP_bool*AC_750))+(1.239e+00*(PP_bool*FO_1000))+(2.038e-02*(PP_bool*VH_150))-(5.588e-03*(PP_bool*TPI_500)))
...
ggplot(QM_TPI_500_Model, mapping=aes(x=UHI_est, y= UHI)) +
  geom_point(alpha = 0.2) +
    labs(x="UHI intensity predicted [K]", 
       y="UHI intensity measured [K]", 
       title= "LUR_B0", #"Interaction model TPI Thun (calibrated on Bern)", 
       subtitle = substitute(
          R^2 == r2 ~ "; RMSE =" ~ rmse ~ "K" ~ "; MBE =" ~ mbe ~ "K",
          list(r2 = R2, rmse = RMSE, mbe = MBE)
        )
  ...
  geom_abline(intercept=0, slope=1, color="red") +
  theme_bw()
ggsave("../results/models/LUR_B0/model_summary.png", width=6.5, height=4, dpi=600)
```
...
for (date in dates) {
  predict_temperature_map(
    ...
    output_path = "../results/models/LUR_T0/",
    uhi_range = c(-1.3, 5)
  )
}
...
# read the model
model <- readRDS("../results/models/LUR_B0/model.rds")
...
# Model with 6 additional predictors
```{r}
source("../R/LUR_int_model.R")
source("../R/map_helpers.R")
source("../R/model_helpers.R")

model_type_short = "LUR_Tplus"
result <- LUR_int_model(
  train_data = Masterfile_train,
  test_data = Masterfile_test,
  target = "UHI",
  target_name = "mean nighttime UHI",
  met_vars = c("WS", "PP_bool", "GR_1d", "CAP27", "RH_1d", "WT", "AT", "WD_8", "PA_1d"),
...
# remove CAP9, GWT8_Z500, CAP27, GWT26_SLP from met_vars_stepwise
met_vars_stepwise <- met_vars_stepwise[!met_vars_stepwise %in% c("CAP9", "GWT8_Z500", "GWT26_SLP", "CAP27", "WD_8_1d")]
#met_vars_stepwise <- met_vars_stepwise[!met_vars_stepwise %in% c("CAP9", "GWT8_Z500", "GWT26_SLP", "GWT26_Z500")]
#met_vars_stepwise <- met_vars_stepwise[!met_vars_stepwise %in% c("CAP27", "GWT8_Z500", "GWT26_SLP", "GWT26_Z500")]
...
# remove WD_8_1d from spat_vars_stepwise
...
Boxplots with model comparisons per measurement site
```{r}
# load model results
results_LUR_B0 <- readRDS("../results/models/LUR_B0/result_list.rds")
results_LUR_T0 <- readRDS("../results/models/LUR_T0/result_list.rds")
results_LUR_Tplus <- readRDS("../results/models/LUR_Tplus/result_list.rds")
results_LUR_Tstep <- readRDS("../results/models/LUR_Tstep/result_list.rds")

residuals_BE <- results_LUR_B0$test_data |>
  select(Log_NR, Location, residuals, LU_7) |>
  mutate(model = "LUR_B0")

residuals_TH <- results_LUR_T0$test_data |>
  select(Log_NR, Location, residuals, LU_7) |>
  mutate(model = "LUR_T0")

residuals_TH_plus <- results_LUR_Tplus$test_data |>
  select(Log_NR, Location, residuals, LU_7) |>
  mutate(model = "LUR_Tplus")

residuals_TH_Step <- results_LUR_Tstep$test_data |>
  select(Log_NR, Location, residuals, LU_7) |>
  mutate(model = "LUR_Tstep")


residuals_all <- bind_rows(residuals_TH, residuals_TH_plus, residuals_TH_Step)

ggplot() +
  geom_hline(yintercept = 0, color = "black", size = 0.5) +
  geom_boxplot(data = residuals_all, aes(x = as.factor(Log_NR), y = residuals, color = model, fill = model),
  ...
  scale_color_manual(values = c("LUR_T0" = "#773399", "LUR_Tplus" = "#449999", "LUR_Tstep" = "#bb4422")) +
  scale_fill_manual(values = c("LUR_T0" = "#bb77dd", "LUR_Tplus" = "#88dddd", "LUR_Tstep" = "#ff8866")) +
  theme_minimal(base_size = 14) +
  # color not in the legend
  guides(color = "none") +
  theme(
...
ggsave("../results/Analysis/residuals_boxplot_per_model.png", width = 10, height = 4.5, dpi = 600)

```


```{r}
# Create order of Log_NR based on LU_7
log_nr_order <- residuals_all |>
  distinct(Log_NR, LU_7) |>
  arrange(LU_7) |>
  pull(Log_NR)

# Apply ordered factor to Log_NR
residuals_all <- residuals_all |>
  mutate(Log_NR = factor(Log_NR, levels = log_nr_order))

ggplot() +
  geom_hline(yintercept = 0, color = "black", size = 0.5) +
  geom_boxplot(data = residuals_all, aes(x = Log_NR, y = residuals, color = model, fill = model),
               outlier.size = 0.8, alpha = 0.9) +
  labs(
    x = "Measurement Site",
    y = "Residuals (Observed - Predicted) [K]",
    fill = "Model"
  ) +
  scale_color_manual(values = c("LUR_T0" = "#773399", "LUR_Tplus" = "#449999", "LUR_Tstep" = "#bb4422")) +
  scale_fill_manual(values = c("LUR_T0" = "#bb77dd", "LUR_Tplus" = "#88dddd", "LUR_Tstep" = "#ff8866")) +
...
ggsave("../results/Analysis/residuals_boxplot_per_model_ordered.png", width = 10, height = 4.5, dpi = 600)
```
