...
all_data_daily_night_clusters |>
  select(time, Log_NR, UHI, time_cluster, station_cluster,
         AT, RH, WS, WD_8, GR_1d, PA, PP_1d)


# Define the variables to normalize
vars_to_normalize <- c("AT", "RH", "WS", "GR_1d", "PA", "PP_1d")
...
normalized_data <- all_data_daily_night_clusters |>
  ...
  mutate(across(all_of(vars_to_normalize), ~ (. - min(., na.rm = TRUE)) / 
                                            (max(., na.rm = TRUE) - min(., na.rm = TRUE)))) |>
  select(time, time_cluster, all_of(vars_to_normalize)) |>
  pivot_longer(cols = all_of(vars_to_normalize), 
               names_to = "variable", values_to = "value") |>
  filter(!is.na(value))
...
ggplot(aes(x = time_cluster, y = value, fill = time_cluster)) +
  ...
  facet_wrap(~variable, scales = "free_y") +
  scale_y_continuous(limits = c(0, 1)) +
  labs(x = "Time Cluster", y = "Normalized Meteorological Variables", title = "Distribution of Normalized Variables by Time Cluster") +
  theme_minimal() +
  theme(legend.position = "none") +
  # arrange the time cluster in the order 0, 3, 1, 2, NA on the x axis
...
# add the four representative nights as point
```{r}
# Define nights to highlight
highlight_nights <- as.Date(c("2024-09-05", "2024-08-14", "2024-08-27", "2024-08-29"))

# Extract the 4 nights' normalized values
highlight_data <- normalized_data |>
  mutate(time = as.Date(time)) |>
  filter(time == "2024-09-05" | time == "2024-08-14" | 
         time == "2024-08-27" | time == "2024-08-29") |>
  filter(!is.na(value)) |>
  mutate(time_cluster = ifelse(time == "2024-09-05", "0",
                                 ifelse(time == "2024-08-14", "3",
                                        ifelse(time == "2024-08-27", "1", "2")))) |>
  group_by(time, variable) |>
  summarize(value = mean(value, na.rm = TRUE), 
            time_cluster = first(time_cluster), .groups = "drop")

# Plot
normalized_data_full |>
  select(time_cluster, all_of(vars_to_normalize)) |>
  pivot_longer(cols = all_of(vars_to_normalize), 
               names_to = "variable", values_to = "value") |>
  filter(!is.na(value)) |>
  mutate(time_cluster = factor(time_cluster, levels = c("0", "3", "1", "2"))) |>
  ggplot(aes(x = time_cluster, y = value, fill = time_cluster)) +
  geom_boxplot(outlier.size = 0.5) +
  # Add the 4 nights as overlaid points
geom_point(data = highlight_data,
           aes(x = factor(time_cluster, levels = c("0", "3", "1", "2")),
               y = value,
               fill = factor(time_cluster)),  # align fill with cluster color
           shape = 21, color = "black", size = 2, stroke = 0.5,
           position = position_jitter(width = 0, height = 0)) +
  facet_wrap(~variable, scales = "free_y") +
  scale_y_continuous(limits = c(0, 1)) +
  labs(x = "Time Cluster", y = "Normalized Meteorological Variables", 
       title = "Distribution of Normalized Variables by Time Cluster",
       subtitle = "White dots: specific nights") +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_x_discrete(limits = c("0", "3", "1", "2", NA)) +
  scale_fill_manual(values = c("0" = "#a6d854", 
                             "3" = "#e78ac3", 
                             "1" = "#fc8d62", 
                             "2" = "#8da0cb",
                             "NA" = "grey90"))  # optional fallback for NA

ggsave(paste0("../results/Clusters/Normalized_Variables_by_Time_Cluster_", timestep, "_highlighted_nights.png"), 
...
highlight_nights_predictors <- meteo_predictors_daily |>
  filter(daynight == "n") |>
  filter(time %in% highlight_nights)
highlight_nights_predictors
```
...
night_colors <- c('#55851A', '#AD3F1B', '#444D83', '#B92A7A')
...
ggplot(plot_data, aes(x = Log_NR)) +
  geom_line(aes(y = UHI_mean, group = time_cluster, color = time_cluster),
  ...
  scale_color_manual(values = cluster_colors,
                     labels = c("0" = "rainy\n2024-09-05\n",
                                ...
                                "3" = "dry & windy\n2024-08-14\n")) +
  theme_minimal() +
  labs( title = "Mean UHI Profiles per Time Cluster", 
  ...
  geom_line(aes(y = UHI_night, group = time_cluster, color = time_cluster),
            linewidth = 0.6) +
  scale_color_manual(values = night_colors, guide = "none") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.4))


ggsave(paste0("../results/Clusters/UHI_Profile_Comparison_", timestep, ".png"), width = 6, height = 4, dpi = 600)
